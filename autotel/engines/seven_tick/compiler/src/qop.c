...
CostModel *create_cost_model(void *engine_state)
{
    ...
    // Cast engine state to actual engine
    EngineState *engine = (EngineState *)engine_state;
    if (!engine)
    {
        // Fallback to defaults if no engine provided
        model->total_triples = 10000;
        model->max_predicate_id = 100;
        model->max_object_id = 10000;
    }
    else
    {
        // Use actual engine statistics
        model->total_triples = engine->triple_count;
        model->max_predicate_id = engine->max_predicate_id;
        model->max_object_id = engine->max_object_id;
    }
    ...
    // Initialize with default selectivities (simplified for now)
