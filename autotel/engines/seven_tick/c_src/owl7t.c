...
// Forward declaration
static int owl_check_transitive_path(OWLEngine* e, uint32_t start, uint32_t property, 
...
int owl_ask_with_reasoning(OWLEngine *e, uint32_t s, uint32_t p, uint32_t o)
{
    ...
    {
        // Implement basic transitive reasoning using depth-first search
        // For performance, limit to reasonable depth
        return owl_check_transitive_path(e, s, p, o, 0, 10); // Max depth 10
    }

    return 0;
}

// Helper function for transitive reasoning
static int owl_check_transitive_path(OWLEngine *e, uint32_t start, uint32_t property,
                                     uint32_t target, int depth, int max_depth)
{
    if (depth > max_depth)
    ...
    for (size_t word = 0; word < intermediates->capacity; word++)
    {
        ...
        while (word_bits)
        {
            ...
            if (owl_check_transitive_path(e, intermediate, property, target, depth + 1, max_depth))
            {
