/*  ─────────────────────────────────────────────────────────────
    benchmark_7t_sql_arm64.c  –  7T-SQL Benchmark Results Demo (ARM64)
    Shows real performance measurements on ARM64
    ───────────────────────────────────────────────────────────── */
...
// Scalar filter implementation (NEON would be used in full implementation)
static uint32_t filter_eq_i32(const int32_t* data, int32_t value, 
                               uint32_t count, uint32_t* out_indices) {
    uint32_t matches = 0;
    
    // Optimized scalar loop with manual unrolling
    uint32_t i;
    for (i = 0; i + 7 < count; i += 8) {
        if (data[i] == value) out_indices[matches++] = i;
        if (data[i+1] == value) out_indices[matches++] = i+1;
        if (data[i+2] == value) out_indices[matches++] = i+2;
        if (data[i+3] == value) out_indices[matches++] = i+3;
        if (data[i+4] == value) out_indices[matches++] = i+4;
        if (data[i+5] == value) out_indices[matches++] = i+5;
        if (data[i+6] == value) out_indices[matches++] = i+6;
        if (data[i+7] == value) out_indices[matches++] = i+7;
    }
...
int main(int argc, char** argv) {
    printf("7T-SQL Real Benchmark Results (ARM64)\n");
    printf("====================================\n\n");
    
    int iterations = (argc > 1) ? atoi(argv[1]) : 10000;
    printf("Running with %d iterations...\n\n", iterations);
    
    // Allocate test data (use malloc for ARM64)
    int32_t* test_data = (int32_t*)malloc(1000 * sizeof(int32_t));
    uint32_t* match_indices = (uint32_t*)malloc(1000 * sizeof(uint32_t));
    
    if (!test_data || !match_indices) {
    ...
    // Benchmark 2: Optimized Filter
    printf("2. Optimized Filter (WHERE value = 42):\n");
    {
        uint64_t min_cycles = UINT64_MAX;
        uint64_t max_cycles = 0;
        uint64_t total_cycles = 0;
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            uint32_t matches = filter_eq_i32(test_data, 42, 1000, match_indices);
    ...
    printf("4. Insert Operation (single row):\n");
    {
        ...
        int32_t* insert_buffer = (int32_t*)malloc(1000 * sizeof(int32_t));
        if (!insert_buffer) {
            printf("Insert buffer allocation failed\n");
            free(test_data);
            free(match_indices);
            return 1;
        }
    ...
    printf("   SIMD: ARM NEON (scalar fallback used)\n");
