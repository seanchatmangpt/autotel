#!/usr/bin/env python3
"""
CNS Weaver 80/20 Fixed Performance Validation
Tests real OpenTelemetry span creation with correct API usage.
"""
...
def create_fixed_benchmark():
    """Create a fixed benchmark that resolves OpenTelemetry API issues."""
    test_code = '''
#include <iostream>
#include <chrono>
#include <vector>
...
// OpenTelemetry includes with explicit namespaces to avoid conflicts
...
namespace otel_exporter = opentelemetry::exporter::trace;
...
int main() {
    std::cout << "🧪 CNS Weaver 80/20 FIXED Performance Test" << std::endl;
    std::cout << "===========================================" << std::endl;
    std::cout << "Testing ACTUAL OpenTelemetry span creation" << std::endl;
    std::cout << std::endl;
    
    // Initialize OpenTelemetry with explicit namespaces and proper shared_ptr handling
    auto exporter = std::make_unique<otel_exporter::OStreamSpanExporter>();
    auto processor = std::make_unique<otel_sdk::SimpleSpanProcessor>(std::move(exporter));
    
    // Use explicit SDK TracerProvider to avoid ambiguity
    auto provider = std::make_shared<otel_sdk::TracerProvider>(std::move(processor));
    
    // Convert std::shared_ptr to nostd::shared_ptr for Provider::SetTracerProvider
    otel_trace::Provider::SetTracerProvider(otel::nostd::shared_ptr<otel_trace::TracerProvider>(provider));
    ...
    if (passed >= (results.size() * 4) / 5) {
        std::cout << "   🎉 80/20 FIXED validation PASSED!" << std::endl;
        std::cout << "   ✅ Weaver spans meet realistic performance requirements" << std::endl;
        return 0;
    } else {
        std::cout << "   ⚠️  80/20 FIXED validation FAILED" << std::endl;
...
'''
    ...
    with open('weaver_80_20_fixed.cpp', 'w') as f:
        f.write(test_code)
    
    print("✅ Created fixed benchmark: weaver_80_20_fixed.cpp")

def compile_fixed_benchmark():
    """Compile the fixed benchmark with correct OpenTelemetry paths."""
    cmd = [
        'g++', '-std=c++17', '-O2', 
        '-I/opt/homebrew/Cellar/opentelemetry-cpp/1.22.0/include',
        '-L/opt/homebrew/Cellar/opentelemetry-cpp/1.22.0/lib',
        '-o', 'weaver_80_20_fixed',
        'weaver_80_20_fixed.cpp',
        '-lopentelemetry_trace',
        '-lopentelemetry_common',
        '-lopentelemetry_resources'
    ]
    
    success, output = run_command(cmd, "Compiling fixed benchmark with OpenTelemetry", timeout=60)
    return success

def run_fixed_benchmark():
    """Run the fixed benchmark."""
    cmd = ['./weaver_80_20_fixed']
    success, output = run_command(cmd, "Running fixed benchmark", timeout=30)
...
int main() {
    printf("🧪 CNS Weaver 80/20 Basic Validation\\n");
    printf("====================================\\n");
...
def main():
    """Main 80/20 fixed performance validation."""
    print("🧪 CNS Weaver 80/20 FIXED Performance Validation")
    print("=" * 55)
    ...
    # Test 3: Fixed OpenTelemetry benchmark
    tests_total += 1
    create_fixed_benchmark()
    if compile_fixed_benchmark():
        success, output = run_fixed_benchmark()
        if success:
            tests_passed += 1
    
    # Summary
    print("\n" + "=" * 55)
    print(f"📊 80/20 FIXED Performance Validation Summary")
    print(f"   Tests passed: {tests_passed}/{tests_total}")
    print(f"   Success rate: {(tests_passed/tests_total)*100:.1f}%")
    
    if tests_passed == tests_total:
        print("   🎉 80/20 FIXED validation completed!")
        print("   ✅ Real weaver performance validated with OpenTelemetry")
        return 0
    else:
        print("   ⚠️  80/20 FIXED validation failed")
