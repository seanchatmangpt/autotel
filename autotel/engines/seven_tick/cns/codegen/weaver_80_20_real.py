#!/usr/bin/env python3
"""
CNS Weaver 80/20 Real Performance Validation
Tests actual OpenTelemetry span creation with realistic benchmarks.
"""
...
def create_real_benchmark():
    """Create a benchmark that tests real OpenTelemetry span creation."""
    test_code = '''
#include <iostream>
#include <chrono>
#include <vector>
...
using namespace opentelemetry::exporters::ostream;
...
PerfResult test_real_span_performance(const std::string& span_name, int iterations) {
    ...
    // Get tracer
    auto provider = TracerProvider::GetTracerProvider();
...
int main() {
    std::cout << "🧪 CNS Weaver 80/20 REAL Performance Test" << std::endl;
    std::cout << "==========================================" << std::endl;
    ...
    TracerProvider::SetTracerProvider(provider);
    ...
    if (passed >= (results.size() * 4) / 5) {
        std::cout << "   🎉 80/20 REAL validation PASSED!" << std::endl;
        std::cout << "   ✅ Weaver spans meet realistic performance requirements" << std::endl;
        return 0;
    } else {
        std::cout << "   ⚠️  80/20 REAL validation FAILED" << std::endl;
...
'''
    ...
    with open('weaver_80_20_real.cpp', 'w') as f:
        f.write(test_code)
    
    print("✅ Created real benchmark: weaver_80_20_real.cpp")

def compile_real_benchmark():
    """Compile the real benchmark with OpenTelemetry."""
    cmd = [
        'g++', '-std=c++17', '-O2', '-o', 'weaver_80_20_real',
        'weaver_80_20_real.cpp',
    ...
    success, output = run_command(cmd, "Compiling real benchmark with OpenTelemetry", timeout=60)
    return success

def run_real_benchmark():
    """Run the real benchmark."""
    cmd = ['./weaver_80_20_real']
...
def create_simple_validation():
    """Create a simple validation that tests basic weaver integration."""
    test_code = '''
#include <stdio.h>
#include <stdint.h>
#include <time.h>
...
'''
    ...
    with open('weaver_simple_validation.c', 'w') as f:
        f.write(test_code)
    
    print("✅ Created simple validation: weaver_simple_validation.c")

def compile_simple_validation():
    """Compile the simple validation."""
    cmd = ['gcc', '-O2', '-I.', '-o', 'weaver_simple_validation', 'weaver_simple_validation.c']
    success, output = run_command(cmd, "Compiling simple validation", timeout=30)
    return success

def run_simple_validation():
    """Run the simple validation."""
    cmd = ['./weaver_simple_validation']
    success, output = run_command(cmd, "Running simple validation", timeout=10)
...
def main():
    """Main 80/20 real performance validation."""
    print("🧪 CNS Weaver 80/20 REAL Performance Validation")
    print("=" * 55)
    ...
    tests_total += 1
    create_simple_validation()
    if compile_simple_validation():
        success, output = run_simple_validation()
        if success:
            tests_passed += 1
    
    # Test 3: Real OpenTelemetry benchmark
    tests_total += 1
    ...
    # Summary
    print("\n" + "=" * 55)
    print(f"📊 80/20 REAL Performance Validation Summary")
    print(f"   Tests passed: {tests_passed}/{tests_total}")
    print(f"   Success rate: {(tests_passed/tests_total)*100:.1f}%")
    
    if tests_passed == tests_total:
        print("   🎉 80/20 REAL validation completed!")
        print("   ✅ Real weaver performance validated with OpenTelemetry")
        return 0
    else:
        print("   ⚠️  80/20 REAL validation failed")
