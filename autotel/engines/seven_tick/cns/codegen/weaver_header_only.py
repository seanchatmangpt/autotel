#!/usr/bin/env python3
"""
CNS Weaver Header-Only Implementation
...
def create_header_only_weaver():
    """Create a header-only weaver that can be included by any code."""
    header_code = '''
#ifndef CNS_WEAVER_H
#define CNS_WEAVER_H
...
// Override base functions with OpenTelemetry integration
#undef cns_perf_start_spqlAsk
#undef cns_perf_end_spqlAsk
#undef cns_perf_start_spqlSelect
#undef cns_perf_end_spqlSelect
#undef cns_perf_start_spqlConstruct
#undef cns_perf_end_spqlConstruct
#undef cns_perf_start_spqlDescribe
#undef cns_perf_end_spqlDescribe
#undef cns_perf_start_spqlUpdate
#undef cns_perf_end_spqlUpdate

// OpenTelemetry-enabled versions
static inline cns_perf_tracker_t cns_perf_start_spqlAsk(void) {
    cns_perf_tracker_t tracker = {0};
    tracker.start_cycles = CNS_GET_CYCLES();
...
'''
    ...
    print("✅ Created header-only weaver: src/cns_weaver.h")
...
def analyze_header_only_benefits():
    ...
    print("   • Function overriding: #undef base functions, redefine with OpenTelemetry")
    print("   • Header-only: No separate compilation units required")

def main():
    """Main header-only weaver implementation."""
    print("🧪 CNS Weaver Header-Only Implementation")
    print("=" * 45)
