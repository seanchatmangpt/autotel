#!/usr/bin/env python3
"""
CNS Weaver 80/20 Simple Performance Validation
Tests real OpenTelemetry span creation with correct include paths.
"""
...
def create_simple_benchmark():
    """Create a simple benchmark that tests real OpenTelemetry span creation."""
    test_code = '''
#include <iostream>
#include <chrono>
#include <vector>
...
// OpenTelemetry includes with correct paths
...
'''
    ...
    with open('weaver_80_20_simple.cpp', 'w') as f:
        f.write(test_code)
    
    print("✅ Created simple benchmark: weaver_80_20_simple.cpp")

def compile_simple_benchmark():
    """Compile the simple benchmark with correct OpenTelemetry paths."""
    cmd = [
        'g++', '-std=c++17', '-O2', 
        '-I/opt/homebrew/Cellar/opentelemetry-cpp/1.22.0/include',
        '-o', 'weaver_80_20_simple',
        'weaver_80_20_simple.cpp',
    ...
    success, output = run_command(cmd, "Compiling simple benchmark with OpenTelemetry", timeout=60)
    return success

def run_simple_benchmark():
    """Run the simple benchmark."""
    cmd = ['./weaver_80_20_simple']
    success, output = run_command(cmd, "Running simple benchmark", timeout=30)
...
def create_basic_validation():
    """Create a basic validation that tests weaver infrastructure."""
    test_code = '''
#include <stdio.h>
#include <stdint.h>
#include <time.h>

// Basic validation of weaver-generated functions
int main() {
...
def main():
    """Main 80/20 simple performance validation."""
    print("🧪 CNS Weaver 80/20 SIMPLE Performance Validation")
    print("=" * 55)
    ...
    # Test 3: Simple OpenTelemetry benchmark
    tests_total += 1
    create_simple_benchmark()
    if compile_simple_benchmark():
        success, output = run_simple_benchmark()
        if success:
            tests_passed += 1
    
    # Summary
    print("\n" + "=" * 55)
    print(f"📊 80/20 SIMPLE Performance Validation Summary")
    print(f"   Tests passed: {tests_passed}/{tests_total}")
    print(f"   Success rate: {(tests_passed/tests_total)*100:.1f}%")
    
    if tests_passed == tests_total:
        print("   🎉 80/20 SIMPLE validation completed!")
        print("   ✅ Real weaver performance validated with OpenTelemetry")
        return 0
    else:
        print("   ⚠️  80/20 SIMPLE validation failed")
