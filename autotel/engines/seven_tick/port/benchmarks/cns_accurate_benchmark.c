/**
 * @file cns_accurate_benchmark.c
 * @brief Accurate performance benchmark for CNS v8 architecture
 *
 * This benchmark provides meaningful performance measurements using:
 * 1. High-resolution timing with proper warmup
 * 2. Statistical analysis (min, max, average)
 * 3. Real-world operation counts
...
#define BENCHMARK_ITERATIONS 10000000 // 10M iterations for better accuracy
#define BENCHMARK_WARMUP_ITERATIONS 100000
#define BENCHMARK_RUNS 5 // Multiple runs for statistical analysis
...
void calculate_statistics(performance_result_t *result)
{
  double sum = 0.0;
  double sum_sq = 0.0;

  result->min_time = result->times[0];
  result->max_time = result->times[0];

  for (int i = 0; i < BENCHMARK_RUNS; i++)
  {
    sum += result->times[i];
    sum_sq += result->times[i] * result->times[i];

    if (result->times[i] < result->min_time)
      result->min_time = result->times[i];
    if (result->times[i] > result->max_time)
      result->max_time = result->times[i];
  }

  result->avg_time = sum / BENCHMARK_RUNS;
  double variance = (sum_sq / BENCHMARK_RUNS) - (result->avg_time * result->avg_time);
  result->std_dev = sqrt(variance);

  result->nanoseconds_per_operation = result->avg_time / result->iterations;
  result->operations_per_second = (double)result->iterations / (result->avg_time / 1000000000.0);
}

void print_performance_result(const char *test_name, const performance_result_t *result)
{
  printf("  %-25s: %8.2f ± %5.2f ns/op, %10.0f ops/sec\n",
         test_name,
         result->nanoseconds_per_operation,
         result->std_dev / result->iterations,
...
void benchmark_owl_subclass_check()
{
  ...
  for (int run = 0; run < BENCHMARK_RUNS; run++)
  {
    ...
    result.times[run] = end_time - start_time;
  }
...
{
  performance_result_t result;
  result.iterations = BENCHMARK_ITERATIONS / 100; // Fewer iterations for memory tests
  ...
  // Calculate bandwidth
  double bytes_per_second = (1024.0 * result.iterations) / (result.avg_time / 1000000000.0);
  double mb_per_second = bytes_per_second / (1024.0 * 1024.0);

  printf("  %-25s: %8.2f ± %5.2f ns/op, %10.0f ops/sec, %8.2f MB/s\n",
...
{
  performance_result_t result;
  result.iterations = BENCHMARK_ITERATIONS / 100;
...
int main()
{
  printf("CNS v8 Architecture - Accurate Performance Benchmark\n");
  printf("====================================================\n\n");
  ...
  printf("Performance Results (mean ± std dev):\n");
  ...
  printf("- High memory bandwidth utilization\n");
