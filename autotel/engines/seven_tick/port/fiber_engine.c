...
static void execute_bytecode(uint64_t actor_id, const uint8_t* bytecode, uint32_t bytecode_size, void* actor_state, void* message) {
    printf("Fiber Engine: Actor %llu executing bytecode (size: %u)...\n", actor_id, bytecode_size);
    // Simulate some work and opcode spans
    l8_record_opcode_span(actor_id, 0xBA_OP_DUMMY_1, 1, 2);
    l8_record_opcode_span(actor_id, 0xBA_OP_DUMMY_2, 3, 4);
...
int fiber_engine_execute_fiber(
    ...
    if (!mailbox->has_message) {
        printf("Fiber Engine: Actor %llu - No message in mailbox. Skipping execution.\n", actor_id);
    ...
    if (entry_point) {
        ...
        // Call the resolved fiber entry point
        entry_point(actor_state, mailbox->message);

        // Execute the bytecode associated with the manifest entry
        execute_bytecode(actor_id, manifest_entry->bytecode, manifest_entry->bytecode_size, actor_state, mailbox->message);

        // Call the behavior callback from the manifest, passing the collector
        if (manifest_entry->num_callbacks > 0 && manifest_entry->behavior_callbacks[0]) {
            manifest_entry->behavior_callbacks[0](actor_state, mailbox->message, collector);
        }

        // Simulate some tick consumption
        elapsed_ticks += manifest_entry->bytecode_size; // Very simplified tick calculation

        // Check time budget (L8 interface)
        if (l8_check_time_budget(actor_id, elapsed_ticks, budget_ticks)) {
        ...
        printf("Fiber Engine: No fiber entry point found for predicate hash %llu.\n", dummy_predicate_hash);
    }
