...
const char *cjinja_get_error_message(CJinjaError error)
{
  ...
  case CJINJA_SUCCESS:
    return "No error";
  case CJINJA_ERROR_NULL_POINTER:
    return "Null pointer";
  case CJINJA_ERROR_MEMORY_ALLOCATION:
    return "Memory allocation failed";
  case CJINJA_ERROR_INVALID_SYNTAX:
    return "Invalid template syntax";
  case CJINJA_ERROR_INVALID_VARIABLE:
    return "Invalid variable";
  case CJINJA_ERROR_INVALID_FILTER:
    return "Invalid filter";
  case CJINJA_ERROR_TEMPLATE_NOT_FOUND:
    return "Template not found";
  case CJINJA_ERROR_LOOP_DEPTH_EXCEEDED:
    return "Loop depth exceeded";
  case CJINJA_ERROR_ARRAY_SIZE_EXCEEDED:
    return "Array size exceeded";
  case CJINJA_ERROR_CACHE_FULL:
    return "Template cache full";
  default:
...
char *cjinja_apply_filter(const char *filter_name, const char *input, const char *args)
{
  ...
  if (strcmp(filter_name, "upper") == 0)
    return cjinja_filter_upper(input, args);
  if (strcmp(filter_name, "lower") == 0)
    return cjinja_filter_lower(input, args);
  if (strcmp(filter_name, "capitalize") == 0)
    return cjinja_filter_capitalize(input, args);
  if (strcmp(filter_name, "trim") == 0)
    return cjinja_filter_trim(input, args);
  if (strcmp(filter_name, "length") == 0)
    return cjinja_filter_length(input, args);
  if (strcmp(filter_name, "default") == 0)
    return cjinja_filter_default(input, args);
      ...
      if (*pos == '|')
      {
        ...
        if (*pos == ':')
        {
          ...
          if (filter_args)
          {
            ...
            while (end > filter_args && isspace(*end))
              *end-- = '\0';
          }
        }
...
{
  if (hit_count)
    *hit_count = g_cache_hits;
  if (miss_count)
    *miss_count = g_cache_misses;
}
...
void cjinja_get_performance_stats(CJinjaEngine *engine, uint64_t *render_count, uint64_t *avg_render_time_ns)
{
  ...
  if (render_count)
    *render_count = engine->render_count;
  if (avg_render_time_ns)
  {
    *avg_render_time_ns = engine->render_count > 0 ? engine->total_render_time_ns / engine->render_count : 0;
  }
}
...
{
  if (!template_str)
    return CJINJA_ERROR_NULL_POINTER;
  ...
  if (brace_count != 0)
    return CJINJA_ERROR_INVALID_SYNTAX;
  if (if_count != 0)
    return CJINJA_ERROR_INVALID_SYNTAX;
  if (for_count != 0)
    return CJINJA_ERROR_INVALID_SYNTAX;
...
int cjinja_has_feature(const char *feature)
{
  ...
  if (strcmp(feature, "loops") == 0)
    return 1;
  if (strcmp(feature, "conditionals") == 0)
    return 1;
  if (strcmp(feature, "filters") == 0)
    return 1;
  if (strcmp(feature, "cache") == 0)
    return 1;
  if (strcmp(feature, "7tick") == 0)
    return 1;
  if (strcmp(feature, "validation") == 0)
    return 1;
  if (strcmp(feature, "performance") == 0)
    return 1;
...
__attribute__((constructor)) static void cjinja_init(void)
{
