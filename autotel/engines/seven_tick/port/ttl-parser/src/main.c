...
// External serializer function declarations
extern bool ttl_serialize_ntriples(ttl_ast_node_t* root, FILE* output);
extern bool ttl_serialize_jsonld(ttl_ast_node_t* root, FILE* output, bool pretty_print);
extern bool ttl_serialize_rdfxml(ttl_ast_node_t* root, FILE* output, bool use_prefixes);
...
// Real format conversion functions using proper serializers
static void output_ntriples(ttl_ast_node_t* ast, FILE* output) {
    if (!ast || !output) return;
    
    // Use the real N-Triples serializer
...
static void output_jsonld(ttl_ast_node_t* ast, FILE* output) {
    ...
    // Use the real JSON-LD serializer
    ttl_serialize_jsonld(ast, output, true); // pretty_print = true
}

static void output_rdfxml(ttl_ast_node_t* ast, FILE* output) {
    if (!ast || !output) return;
    
    // Use the real RDF/XML serializer
    ttl_serialize_rdfxml(ast, output, true); // use_prefixes = true
}
