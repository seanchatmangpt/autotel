#!/usr/bin/env python3
"""
Quiet OntologyCompiler test with telemetry disabled.
"""
...
def test_ontology_compiler_quiet():
    """Test OntologyCompiler with telemetry disabled."""
    
    print("🧪 Testing OntologyCompiler (Quiet)")
    print("=" * 40)
    ...
    try:
        ...
        # Parse OWL with processor (telemetry disabled)
        print("\n🔍 Parsing OWL with OWLProcessor...")
        from autotel.core.telemetry import create_telemetry_manager
        
        # Create telemetry manager with tracing disabled
        telemetry = create_telemetry_manager(
            service_name="autotel-owl-processor-quiet",
            enable_tracing=False,
            enable_metrics=False,
            require_linkml_validation=False
        )
        
        # Create processor with custom telemetry
        processor = OWLProcessor()
        processor.telemetry = telemetry
        
        ontology_def = processor.parse_ontology_definition(xml_content, prefix='quiet_test')
        ...
        # Compile with OntologyCompiler (telemetry disabled)
        print("\n⚙️ Compiling with OntologyCompiler...")
        
        # Create telemetry manager with tracing disabled
        telemetry = create_telemetry_manager(
            service_name="autotel-ontology-compiler-quiet",
            enable_tracing=False,
            enable_metrics=False,
            require_linkml_validation=False
        )
        
        # Create compiler with custom telemetry
        compiler = OntologyCompiler()
        compiler.telemetry = telemetry
        ...
        output_file = "compiled_ontology_quiet.json"
...
if __name__ == "__main__":
    success = test_ontology_compiler_quiet()
    if success:
        print("\n🎉 OntologyCompiler quiet test PASSED!")
    else:
        print("\n💥 OntologyCompiler quiet test FAILED!")
        sys.exit(1) 